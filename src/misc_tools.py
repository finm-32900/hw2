...
def df_to_literal(df, missing_value="None"):
    ...
    missing_value : str, default="None"
        String to use for representing missing values in the output.
    ...
    With named columns:
    >>> df = pd.DataFrame({
    ...     "date": [pd.Timestamp("2022-08-31"), pd.Timestamp("2022-09-30")],
    ...     "value": [1.5, 2.5]
    ... }, index=[1, 2])
    ...
    pd.DataFrame({
        "date": [pd.Timestamp("2022-08-31"), pd.Timestamp("2022-09-30")],
        "value": [1.5, 2.5]
    }, index=[1, 2], columns=pd.Index(["date", "value"], name="metrics"))

    Notes
    -----
    The function preserves:
    - Column names and data
    - Index values and name (if not default RangeIndex)
    - Column index name (if exists)
    - None values (converted from NaN)
    - Datetime values (converted to Timestamp strings)
    """
    # Convert to dictionary while preserving datetime format
    cols = {}
    for col in df.columns:
        if df[col].dtype == 'datetime64[ns]':
            # Handle datetime columns without quotes around pd.Timestamp
            cols[col] = [f'pd.Timestamp("{x}")' if pd.notnull(x) else missing_value 
                        for x in df[col].dt.strftime('%Y-%m-%d')]
            # Remove the quotes that were added by the list representation
            cols[col] = str(cols[col]).replace("'pd.Timestamp(", "pd.Timestamp(").replace(")'", ")")
        else:
            # Handle other columns
            values = df[col].tolist()
            # Convert string values to use double quotes
            if df[col].dtype == 'object':
                values = [f'"{x}"' if isinstance(x, str) else str(x) for x in values]
            cols[col] = str(values).replace("'None'", "None").replace("'", "")
    ...
    if (
        ...
        # Convert index values to use double quotes if they're strings
        if isinstance(df.index, pd.Index):
            index_values = [f'"{x}"' if isinstance(x, str) else str(x) for x in index_values]
            index_values = str(index_values).replace("'", "")
        if df.index.name:
            lines[-1] += f', index=pd.Index({index_values}, name="{df.index.name}")'
        else:
            lines[-1] += f", index={index_values}"
    else:
        # Add explicit index values for RangeIndex
        index_values = list(df.index)
        lines[-1] += f", index={index_values}"

    # Add columns index name if it exists
    if df.columns.name:
        column_values = [f'"{x}"' for x in df.columns]
        column_list = str(column_values).replace("'", "")
        lines[-1] += f', columns=pd.Index({column_list}, name="{df.columns.name}")'
