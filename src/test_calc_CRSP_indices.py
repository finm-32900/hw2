...
def test_calc_value_idx():
    ...
    assert_frame_equal(
        calc_CRSP_indices.calc_CRSP_value_weighted_index(input, freq="MS").round(4),
...
def test_calc_CRSP_indices_merge():
    ...
    assert_frame_equal(
        calc_CRSP_indices.calc_CRSP_indices_merge(input, idx_input, freq="MS").round(4),
        expected_output.round(4),
    )


def test_compare_CRSP_manual():
    VW_AVE_THRESHOLD = 0.003
    VW_MAX_THRESHOLD = 0.02
    EW_AVE_THRESHOLD = 0.02
    ...
    # threshold to 0.003 to be safe. Additionally, the difference
    ...
    # the securities are weighted evenly (and we don't know what the security 
    # universe CRSP is using is).
    ...
    # TEST 3: Make sure that the total security count is within 30% of the
    # manual calculation. 
    # ## TODO: This needs to be fixed. 30% is too high.
    security_count_percent_diff = (df["totcnt"] - df["totcnt_manual"]) / df["totcnt_manual"]
    security_count_percent_diff.plot()
    assert (
        security_count_percent_diff.abs() < 0.30
    ).all()

    # TEST 4: Test that cumulative returns are within 7% of each other per year
    # for the value-weighted index and within 25% per year for the equally weighted index.
    # This is a bit arbitrary, but it's a good sanity check.
    ...
    # TEST 5: Make sure that they are  > 99.6% correlated.
    assert df[["vwretd", "vwretd_manual"]].corr().iloc[0, 1] > 0.996
    assert df[["vwretx", "vwretx_manual"]].corr().iloc[0, 1] > 0.996
    assert df[["ewretd", "ewretd_manual"]].corr().iloc[0, 1] > 0.996    
    assert df[["ewretx", "ewretx_manual"]].corr().iloc[0, 1] > 0.996